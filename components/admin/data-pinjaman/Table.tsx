...
export function DataPinjamanTable() {
  ...
  const hasRequestStatus = pinjamanData.some((pinjaman) => pinjaman.status === "request")
  const hasNonRequestStatus = pinjamanData.some(
    (pinjaman) => pinjaman.status !== "request" && pinjaman.status !== "refused" && pinjaman.status !== "done",
  )
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-violet-50">
                ...
                {hasRequestStatus && <TableHead className="w-24 text-center">Aksi</TableHead>}
                {hasNonRequestStatus && <TableHead className="w-24 text-center">Aksi</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
              {pinjamanData.length === 0 ? (
                ...
                pinjamanData.map((pinjaman) => (
                  <TableRow key={pinjaman.id} className="hover:bg-violet-50/50">
                    <TableCell className="text-center font-medium">{pinjaman.id}</TableCell>
                    ...
                    <TableCell>
                      <div className="font-medium text-gray-900">{pinjaman.buku?.title}</div>
                    </TableCell>
                    <TableCell>
                      <div className="font-medium text-gray-700">{pinjaman.user?.name}</div>
                    </TableCell>
                    <TableCell className="text-center">
                      <span className="text-sm text-gray-600">{pinjaman.durasi_pinjaman} hari</span>
                    </TableCell>
                    <TableCell className="text-center">
                    ...
                    <TableCell className="text-center text-sm text-gray-600">
                      {shouldShowTanggalDipinjam(pinjaman.status) ? formatDate(pinjaman.tanggal_dipinjam) : "-"}
                    </TableCell>
                    <TableCell className="text-center text-sm text-gray-600">
                      {shouldShowTanggalDikembalikan(pinjaman.status) ? formatDate(pinjaman.tanggal_dikembalikan) : "-"}
                    </TableCell>
                    {hasRequestStatus && (
                      <TableCell className="text-center">
                      ...
                      <TableCell className="text-center">
                        {shouldShowAksi(pinjaman.status) ? (
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                <AlertDialogTitle>Selesaikan Pinjaman?</AlertDialogTitle>
                                <AlertDialogDescription>
                                  Apakah Anda yakin ingin menyelesaikan pinjaman buku {pinjaman.buku?.title} oleh{" "}
                                  {pinjaman.user?.name}? Tindakan ini akan mengembalikan stok buku dan mengubah status
                                  pinjaman menjadi selesai.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel>Batal</AlertDialogCancel>
                                <AlertDialogAction onClick={() => handleComplete(pinjaman.id)}>
                                  Selesaikan
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
