...
export default function SheetEditBuku({ open, onOpenChange, buku, onSuccess }: SheetEditBukuProps) {
  ...
  const onSubmit = async (data: BookEditData) => {
    try {
      ...
      if (data.cover && typeof data.cover !== "string") {
        ...
        if (!uploadRes.ok) throw new Error(uploadData.error || "Gagal upload cover");
      ...
      const errorMessage = error instanceof Error ? error.message : "Gagal update buku";
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="right" className="max-w-lg w-full p-6 overflow-y-auto text-gray-700">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-6">
            ...
            <div className="space-y-2">
              ...
              {errors.rak && <p className="text-sm text-red-500">{errors.rak.message}</p>}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              {errors.sinopsis && <p className="text-sm text-red-500">{errors.sinopsis.message}</p>}
            </div>
            {/* Kategori */}
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" onClick={() => {
                  const input = document.querySelector<HTMLInputElement>("#kategori-input");
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {watchedCategories.map((cat, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-violet-100 text-violet-800 px-3 py-1 rounded-full">
                    <span className="text-sm">{cat}</span>
                    <button type="button" onClick={() => handleRemoveCategory(cat)} className="text-violet-600 hover:text-violet-800">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ))}
            ...
            {/* Cover (opsional) */}
            <div className="space-y-2">
              <Label htmlFor="cover">Upload Cover Baru (opsional)</Label>
            ...
            <div className="flex gap-4 pt-4">
              <Button type="submit" disabled={isSubmitting || isUploading} className="bg-violet-600 hover:bg-violet-700">
                {isSubmitting || isUploading ? "Menyimpan..." : "Simpan Perubahan"}
              </Button>
              <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="border-violet-600 text-violet-600 hover:bg-violet-50">
                Batal
              </Button>
            </div>
          </form>
